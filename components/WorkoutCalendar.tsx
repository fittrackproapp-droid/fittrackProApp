...
const WorkoutCalendar: React.FC<WorkoutCalendarProps> = ({
  ...
  for (let d = 1; d <= daysInMonth; d++) {
    ...
    days.push(
      ...
      >
        <span
          className={`text-sm ${indicator ? "font-bold text-gray-900" : "text-gray-500"}`}
        >
          {d}
        </span>
        {indicator && (
    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
      ...
      <div className="grid grid-cols-7 text-center border-b border-gray-100">
        {weekDays.map((d) => (
          <div
            key={d}
            className="py-2 text-xs font-bold text-gray-400 uppercase"
          >
            {t(d)}
          </div>
        ))}
      </div>
      <div className="grid grid-cols-7 p-2 gap-1">{days}</div>
      <div className="p-2 flex gap-4 text-xs justify-center border-t border-gray-100 bg-gray-50/50">
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-green-500"></div>{" "}
          {t("col_status")}: {t("status_completed")}
        </div>
        <div className="flex items-center gap-1">
          <div className="w-2 h-2 rounded-full bg-yellow-400"></div>{" "}
          {t("col_status")}: {t("status_pending")}
        </div>
      </div>
    </div>
  );
