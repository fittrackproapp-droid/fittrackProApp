...
import { autoSplitIfNeeded, SplitProgress } from "./services/videoUtils";
import {
...
> = ({ className, variant = "primary", ...props }) => {
  const base =
    "px-4 py-2 rounded-lg font-medium transition flex items-center justify-center gap-2 active:scale-95 disabled:opacity-50 disabled:active:scale-100";
  const variants = {
    primary:
...
const Input: React.FC<React.InputHTMLAttributes<HTMLInputElement>> = ({
  ...
  <input
    className={`w-full px-4 py-2 rounded-lg border border-gray-200 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-200 outline-none transition ${className || ""}`}
    {...props}
  />
);
...
const Badge: React.FC<{ children: React.ReactNode; color: string }> = ({
  ...
  <span
    className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wide ${color}`}
  >
    {children}
  </span>
);
  ...
  switch (type.toLowerCase()) {
    case "chest": // Pectorals
      return (
        <svg {...commonProps}>
          <path
            d="M4 7c0 3 2 5 5 5s5-2 5-5c0-2-2-4-5-4S4 5 4 7z"
            opacity="0.5"
          />
          <path
            d="M15 7c0 3 2 5 5 5s5-2 5-5c0-2-2-4-5-4s-5 2-5 4z"
            opacity="0.5"
          />
...
const AuthScreen = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(
        err.message || "Authentication failed. Check your Firebase Config.",
      );
    } finally {
      setLoading(false);
    }
      ...
      <div className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-md border border-gray-100">
        <div className="text-center mb-8">
          ...
          <p className="text-gray-500 text-sm mt-1">
            {isLogin ? t("sign_in_title") : t("create_account_title")}
          </p>
        </div>
...
const PlanDisplay = ({
  ...
  const planExercises = exercises.filter((e) =>
    plan.exerciseIds.includes(e.id),
  );

  return (
    <>
      <p className="text-indigo-100 text-sm mb-6 flex items-center gap-2">
        <CheckCircle size={14} /> {t("assigned_by")}{" "}
        {users.find((u) => u.id === plan.coachId)?.name}
      </p>
        ...
        {planExercises.map((ex) => (
          ...
          >
            <div className="flex items-center gap-3">
              <div
                className={`p-1 rounded-full ${recordingExercises.includes(ex.id) ? "bg-indigo-100 text-indigo-600" : "bg-white/20 text-white"}`}
              >
              ...
              <div>
                ...
                <span
                  className={`text-[10px] ${recordingExercises.includes(ex.id) ? "text-indigo-400" : "text-indigo-200"}`}
                >
                  {t(ex.category)}
                </span>
              </div>
            </div>
            {recordingExercises.includes(ex.id) ? (
              <CheckCircle className="text-indigo-600" size={20} />
            ) : (
              <div className="w-5 h-5 rounded-full border-2 border-indigo-200/30" />
            )}
          </div>
        ))}
      </div>
      ...
      >
        <Video size={18} />
        {recordingExercises.length === 0
          ? t("select_to_start")
          : `${t("record_session")} (${recordingExercises.length})`}
      </Button>
    </>
  );
};
          ...
          <table className="w-full text-sm text-left rtl:text-right">
            ...
            <tbody className="divide-y divide-gray-100">
              {trainees.map((u, index) => (
                <tr
                  key={u.id}
                  className={`${u.id === currentUser.id ? "bg-indigo-50" : "hover:bg-gray-50"} transition`}
                >
                  ...
                  <td className="px-6 py-4 font-bold text-gray-900 flex items-center gap-3">
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center text-xs text-white font-bold ${u.id === currentUser.id ? "bg-indigo-600" : "bg-gray-400"}`}
                    >
                      {u.name.charAt(0)}
                    </div>
                    {u.name}{" "}
                  ...
                  <td className="px-6 py-4 text-center">
                    <Badge color="bg-amber-100 text-amber-800 text-sm px-3 py-1">
                      {u.points} pts
                    </Badge>
                  </td>
                </tr>
...
const App = () => {
  ...
  const [splitPhase, setSplitPhase] = useState<SplitProgress | null>(null);
  ...
  const [removeMediaConfirm, setRemoveMediaConfirm] = useState<string | null>(
    null,
  );
  ...
  const requestPermission = async () => {
    ...
    if (res === "granted" && user) {
      ...
      await setDoc(
        doc(dbFirestore, "pushSubscriptions", user.id),
        {
  ...
  const sendPushToUser = async (
    ...
    try {
      const subDoc = await getDoc(
        doc(dbFirestore, "pushSubscriptions", targetUserId),
      );
      if (!subDoc.exists()) {
        ...
        if (prevSubs.length > 0 && currentUser) {
          ...
          if (currentUser.role === UserRole.COACH) {
            const newPending = newSubs.filter(
              (s) =>
                s.status === "PENDING" && !prevSubs.find((p) => p.id === s.id),
            );
            if (newPending.length > 0) {
              const traineeName =
                currentUsers.find((u) => u.id === newPending[0].traineeId)
                  ?.name || "Trainee";
          ...
          if (currentUser.role === UserRole.TRAINEE) {
            const newlyCompleted = newSubs.filter(
              (s) =>
            ...
            if (newlyCompleted.length > 0) {
              const reviewedExerciseNames = currentExercises
                ...
                .join(", ");
              sendLocalNotification(
                t("push_workout_reviewed_title"),
                `${t("msg_exercises")} ${reviewedExerciseNames}`,
              );
              sendPushToUser(
                newlyCompleted[0].traineeId,
                t("push_workout_reviewed_title"),
                reviewedExerciseNames,
              );
            }
          }
        }
  ...
  const handleMarkRead = async (messageIds: string[]) => {
    await db.markMessagesAsRead(messageIds);
    refreshData();
  };
  ...
  const handleRoleChange = async (userId: string, newRole: UserRole) => {
    ...
    try {
      await db.updateUser({
        ...
        role: newRole,
        coachId:
          newRole === UserRole.COACH || newRole === UserRole.ADMIN
            ? null
            : u.coachId,
      });
      await refreshData();
  ...
  const handleUpdatePoints = async (userId: string, points: number) => {
    const u = users.find((x) => x.id === userId);
    if (u) {
      await db.updateUser({ ...u, points });
      refreshData();
    }
  };
  const handleDeleteUser = (userId: string) => {
    setDeleteUserConfirm(userId);
  };
  ...
  const handleAssignCoach = async (traineeId: string, coachId: string) => {
    const u = users.find((x) => x.id === traineeId);
    if (u) {
      await db.updateUser({ ...u, coachId });
      refreshData();
    }
  };
  const handleTraineeClick = (traineeId: string) => {
    setSelectedTraineeId(traineeId);
    setView("TRAINEE_DETAILS");
    setSelectedDate(null);
  };
  const handleEditProgram = async () => {
    setLoadingData(true);
    const plan = await db.getPlan(selectedTraineeId);
    setCurrentPlanExercises(plan ? plan.exerciseIds : []);
    setView("PLANNING");
    setLoadingData(false);
  };
  const handleOpenInbox = (traineeId: string) => {
    setSelectedTraineeId(traineeId);
    setView("INBOX");
  };
  const saveTraineePlan = async () => {
    if (!selectedTraineeId || !user) return;
    const plan: WorkoutPlan = {
      id: crypto.randomUUID(),
      coachId: user.id,
      traineeId: selectedTraineeId,
      exerciseIds: currentPlanExercises,
  ...
  const inviteTrainee = (traineeId: string) => {
    handleAssignCoach(traineeId, user!.id);
  };
  const markCompleted = async (
    ...
    const exerciseNames = exercises
      .filter((e) => sub.exerciseIds.includes(e.id))
      .map((e) => t(e.name))
      .join(", ");
      ...
      for (const vidId of vids) {
        ...
        if (url) {
          try {
            ...
            console.error("Failed to fetch video for edit", fetchErr);
          }
        }
      }
    } catch (e) {
      console.error("Error loading existing videos for edit", e);
    }
    setSessionMedia(mediaItems);
    setView("SESSION_HUB");
    setLoadingData(false);
  };
  const toggleFeedback = (subId: string) => {
    if (expandedFeedback.includes(subId)) {
      setExpandedFeedback((prev) => prev.filter((id) => id !== subId));
    } else {
  ...
  const handleFinishWorkout = async () => {
    ...
    try {
      ...
      // ── Phase 1: Auto-split large videos ──────────────────────────────────
      // For each new (non-existing) media item that exceeds 82 MB, we
      // re-encode it into smaller segments before uploading.
      // Results are stored in parallel with sessionMedia so order is preserved.
      ...
      // null  → existing video (use media.id directly)
      // array → one or more blobs to upload for this item
      const uploadEntriesPerItem: (UploadEntry[] | null)[] = [];
      const newMediaItems = sessionMedia.filter((m) => !m.isExisting);
      let newFilesSeen = 0;

      for (const media of sessionMedia) {
        if (media.isExisting) {
          uploadEntriesPerItem.push(null);
          continue;
        }

        const fileIndex = newFilesSeen;
        newFilesSeen++;

        const blobs = await autoSplitIfNeeded(
          media.file,
          fileIndex,
        ...
        uploadEntriesPerItem.push(
          blobs.map((b, idx) => ({
            blob: b,
            name:
              blobs.length > 1
                ? `${media.name} (part ${idx + 1} of ${blobs.length})`
                : media.name,
          })),
        );
      }
      ...
      // ── Phase 2: Upload all blobs ─────────────────────────────────────────
      const allEntries = uploadEntriesPerItem
        .filter((e): e is UploadEntry[] => e !== null)
        .flat();
      const totalNew = allEntries.length;
      ...
      for (let i = 0; i < sessionMedia.length; i++) {
        const media = sessionMedia[i];
        const entries = uploadEntriesPerItem[i];

        if (entries === null) {
          // Already-existing video — keep its ID
        ...
        for (const entry of entries) {
          ...
          const vid = await saveVideo(entry.blob, (filePct) => {
            setUploadFileProgress(filePct);
            const overall =
              ((newFilesDone - 1 + filePct / 100) / totalNew) * 100;
      ...
      if (user.coachId) {
        const exerciseNames = exercises
          .filter((e) => recordingExercises.includes(e.id))
          .map((e) => t(e.name as TranslationKey))
          .join(", ");
  ...
  const renderSubmissionsList = (
    ...
    if (subs.length === 0)
      return (
        <p className="text-gray-500 italic p-4 text-center text-sm">
    ...
    return subs.map((sub) => {
      ...
      const subExercises = exercises.filter((e) =>
        sub.exerciseIds.includes(e.id),
      );
      ...
      return (
        <div
          key={sub.id}
          className="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-3 transition"
        >
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            {subExercises.map((e) => (
              <span
                key={e.id}
                className="text-[10px] bg-gray-100 text-gray-700 px-1.5 py-0.5 rounded border border-gray-200 flex items-center gap-1"
              >
              ...
              videoIds.map((vid, idx) => {
                if (vid === "DELETED") {
                  return (
                    <span
                      key={idx}
                      className="px-2 py-1 bg-gray-100 text-gray-400 rounded text-xs border border-gray-200 flex items-center gap-1 cursor-default"
                    >
                ...
                return (
                  <div key={idx} className="flex items-center gap-2">
                    <button
                      ...
                      className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-600 rounded-lg hover:bg-indigo-100 text-xs font-bold transition border border-indigo-100 shadow-sm"
                    >
                      <Play size={14} />{" "}
                      {videoIds.length > 1
                        ? `${t("video_label")} ${idx + 1}`
          ...
          {isCoach && isPending && (
            <div className="mt-4 pt-3 border-t border-gray-100 space-y-3">
              <textarea
                id={`feedback-${sub.id}`}
                className="w-full text-sm px-3 py-2 border rounded-lg bg-gray-50 focus:bg-white outline-none min-h-[80px]"
              ...
              <div className="flex justify-between items-center gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <input
                    type="number"
                    id={`points-${sub.id}`}
                    defaultValue={sub.exerciseIds.length}
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    markCompleted(sub, fbVal, ptsVal);
                  }}
                >
                  {t("approve")}
                </Button>
                ...
                <div>
                  ...
                  >
                    {expandedFeedback.includes(sub.id) ? (
                      <>
                        <ChevronUp size={14} /> {t("hide_feedback")}
                      </>
                    ) : (
                      <>
                        <ChevronDown size={14} /> {t("view_feedback")}
                      </>
                    )}
                  </button>
                  {expandedFeedback.includes(sub.id) && (
  ...
  if (!user)
    return <AuthScreen onLogin={setUser} t={t} lang={lang} setLang={setLang} />;
  const ChevronNext = lang === "he" ? ChevronLeft : ChevronRight;
  const ChevronPrev = lang === "he" ? ChevronRight : ChevronLeft;

  return (
    <div
      className="min-h-screen flex flex-col bg-slate-50 font-sans text-slate-900 pb-16"
      dir={lang === "he" ? "rtl" : "ltr"}
    >
        ...
        <div className="max-w-5xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              className={`p-2 rounded hover:bg-gray-100 text-gray-600 ${view === "LEADERBOARD" ? "text-indigo-600 bg-indigo-50" : ""}`}
              title={t("leaderboard")}
            >
              <Trophy size={20} />
            </button>
            <button
              onClick={handleHomeView}
              className={`p-2 rounded hover:bg-gray-100 text-gray-600 ${(user.role === UserRole.ADMIN && view === "ADMIN") || (user.role !== UserRole.ADMIN && view === "DASHBOARD") ? "text-indigo-600 bg-indigo-50" : ""}`}
              title={t("home")}
            >
              <Home size={20} />
            </button>
            {(user.role === UserRole.TRAINEE ||
              ...
              <button
                ...
                className={`relative p-2 rounded hover:bg-gray-100 text-gray-600 ${view === "INBOX" ? "text-indigo-600 bg-indigo-50" : ""}`}
              >
        ...
        <div className="fixed inset-0 z-50 bg-slate-500/50 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="w-full max-w-2xl bg-white rounded-2xl overflow-hidden relative shadow-2xl border border-slate-200">
            <button
              ...
              className="absolute top-4 right-4 rtl:left-4 rtl:right-auto text-slate-600 hover:text-slate-900 bg-white/50 hover:bg-white p-2 rounded-full z-10 transition"
            >
              ✕
            </button>
            <div className="p-1 bg-slate-100">
              <video
                ...
                controls
                autoPlay
                className="w-full max-h-[80vh] bg-slate-300 rounded-lg"
              />
            </div>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-gray-200 p-6 w-full max-w-sm text-center">
            ...
            <h3 className="text-lg font-bold text-gray-900 mb-1">
              {t("delete_video_title")}
            </h3>
            <p className="text-sm text-gray-500 mb-6">
              {t("delete_video_warning")}
            </p>
              ...
              <button
                onClick={() => {
                  setSessionMedia((prev) =>
                    prev.filter((m) => m.id !== removeMediaConfirm),
                  );
              ...
              <div className="flex flex-col items-center justify-center py-2 space-y-4">
                <div className="relative w-24 h-24 flex items-center justify-center">
                  ...
                  >
                    ...
                    <circle
                      ...
                      strokeDashoffset={
                        263.9 -
                        (263.9 * Math.min(splitPhase.overallProgress, 100)) /
                          100
                      }
                    />
                  </svg>
                ...
                <div className="text-center">
                  <h3 className="text-lg font-bold text-gray-900">
                    {splitPhase.totalFiles > 1
                      ? t("preparing_videos")
                      : t("preparing_video")}
                  </h3>

                  {splitPhase.phase === "measuring" ? (
                    ...
                    <>
                      <p className="text-sm text-gray-500 mt-1">
                        {splitPhase.totalFiles > 1 &&
                          `${t("file_label")} ${splitPhase.fileIndex + 1}/${splitPhase.totalFiles} · `}
                        {t("clip_label")} {splitPhase.segmentIndex + 1}{" "}
                        {t("splitting_of")} {splitPhase.totalSegments}
                      </p>

                      <div className="w-56 mt-3 space-y-1.5 text-left">
                        <div className="flex justify-between items-center">
                          <span className="text-xs text-gray-400">
                            {t("clip_label")} {splitPhase.segmentIndex + 1}/
                            {splitPhase.totalSegments}
                          </span>
                          <span className="text-xs font-bold text-amber-600 tabular-nums">
                            {Math.round(splitPhase.segmentProgress)}%
                          </span>
                        </div>
                        <div className="h-2 bg-gray-100 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(splitPhase.segmentProgress, 100)}%`,
                            }}
                          />
                        </div>

                        {splitPhase.totalSegments > 1 && (
                          <>
                            <div className="flex justify-between items-center pt-1">
                              ...
                              <span className="text-xs text-gray-400 tabular-nums">
                                {Math.round(splitPhase.overallProgress)}%
                              </span>
                            </div>
                            <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                              <div
                                className="h-full bg-amber-300 rounded-full transition-all duration-300 ease-out"
                                style={{
                                  width: `${Math.min(splitPhase.overallProgress, 100)}%`,
                                }}
                              />
                            </div>
                          </>
                  ...
                  <p className="text-xs text-gray-400 mt-3">
                    {t("splitting_realtime_note")}
                    {splitPhase.totalSegments > 1 &&
                      ` (${splitPhase.totalSegments} ${t("splitting_clips")})`}
                  </p>
                </div>
              </div>
            ) : isUploading ? (
      ...
      <main className="flex-1 max-w-5xl mx-auto w-full p-4 md:p-6 space-y-6">
        ...
        {view === "LEADERBOARD" && (
          <Leaderboard users={users} currentUser={user} t={t} />
        )}
        {(view === "ADMIN" ||
            ...
            <div className="space-y-6">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className={`px-4 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2 ${adminView === "USERS" ? "bg-indigo-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}
                >
                  <Users size={16} /> {t("manage_users")}
                </button>
                <button
                  onClick={() => setAdminView("SUBMISSIONS")}
                  className={`px-4 py-2 rounded-lg font-bold text-sm transition flex items-center gap-2 ${adminView === "SUBMISSIONS" ? "bg-indigo-600 text-white" : "bg-white text-gray-600 hover:bg-gray-50"}`}
                >
                  <List size={16} /> {t("all_submissions")}
                </button>
              </div>
              {adminView === "USERS" && (
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    {users
                      ...
                      .map((u) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            <div
                              className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold ${u.role === UserRole.COACH ? "bg-purple-100 text-purple-600" : u.role === UserRole.ADMIN ? "bg-gray-800 text-white" : "bg-green-100 text-green-600"}`}
                            >
                              {u.name.charAt(0)}
                            </div>
                            <div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between max-w-3xl mx-auto">
              <h2 className="text-xl font-bold flex items-center gap-2">
                <Mail className="text-indigo-600" /> {t("inbox")}
              </h2>
          ...
          <div className="max-w-3xl mx-auto space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              <h3 className="font-bold text-gray-800 mb-6 flex items-center gap-2">
                <Users className="text-indigo-600" /> {t("my_trainees")}
              </h3>
              <div className="space-y-3">
                {users
                  .filter((u) => u.coachId === user.id)
                  .map((trainee) => (
                    <div
                      key={trainee.id}
                      onClick={() => handleTraineeClick(trainee.id)}
                      className="flex items-center justify-between p-4 rounded-xl bg-gray-50 hover:bg-indigo-50 transition border border-gray-100 hover:border-indigo-200 group cursor-pointer shadow-sm hover:shadow-md"
                    >
                      <div className="flex items-center gap-4">
                        <div className="w-12 h-12 rounded-full bg-white text-indigo-600 flex items-center justify-center font-bold text-lg shadow-sm">
                          {trainee.name.charAt(0)}
                        </div>
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            ...
                            {t("reviews_pending_label")} • {trainee.points} pts
                          </p>
                        </div>
                      </div>
                      <ChevronNext
                ...
                {users.filter((u) => u.coachId === user.id).length === 0 && (
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <div className="space-y-3">
                {users
                  .filter((u) => u.role === UserRole.TRAINEE && !u.coachId)
                  .map((trainee) => (
                    <div
                      key={trainee.id}
                    ...
                    >
                      ...
                      <button
                        onClick={() => inviteTrainee(trainee.id)}
                        className="text-xs bg-indigo-600 text-white px-3 py-1.5 rounded-lg hover:bg-indigo-700 font-bold transition shadow-sm shadow-indigo-200"
                      >
                        {t("add")}
                      </button>
                    </div>
                  ))}
                {users.filter((u) => u.role === UserRole.TRAINEE && !u.coachId)
                  .length === 0 && (
        ...
        {view === "TRAINEE_DETAILS" && user.role === UserRole.COACH && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              >
                <ChevronPrev size={16} /> {t("back_dashboard")}
              </button>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 flex flex-col md:flex-row justify-between items-center gap-4">
              <div className="flex items-center gap-4">
                <div className="w-16 h-16 rounded-full bg-indigo-100 text-indigo-700 flex items-center justify-center font-bold text-2xl">
                  {users
                    .find((u) => u.id === selectedTraineeId)
                    ?.name.charAt(0)}
                </div>
                <div>
                  <h2 className="text-2xl font-bold text-gray-900">
                    {users.find((u) => u.id === selectedTraineeId)?.name}
                  </h2>
                  <p className="text-gray-500 text-sm flex items-center gap-1">
                    <Trophy size={14} />{" "}
                    {users.find((u) => u.id === selectedTraineeId)?.points}{" "}
                    {t("points_lowercase")}
                  </p>
                </div>
              </div>
              <div className="flex gap-2">
                <Button
                  onClick={() => handleOpenInbox(selectedTraineeId)}
                  variant="primary"
                >
                  <MessageSquare size={16} /> {t("send_message")}
                </Button>
                <Button onClick={handleEditProgram} variant="secondary">
                  <Edit size={16} /> {t("edit_program")}
                </Button>
              </div>
            </div>
              ...
              <div className="lg:col-span-2 space-y-6">
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                  <h3 className="font-bold text-gray-800 mb-4 flex items-center gap-2">
                    <CalendarIcon size={18} /> {t("completed_workouts")}
                  </h3>
                  <WorkoutCalendar
                    submissions={submissions.filter(
                      (s) => s.traineeId === selectedTraineeId,
                    )}
                ...
                {selectedDate && (
                  <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 animate-in fade-in slide-in-from-top-4 duration-300">
                    <h4 className="font-bold text-gray-800 mb-3 text-sm flex justify-between items-center">
                      <span>
                        {t("workouts_on")} {selectedDate.toLocaleDateString()}
                      </span>
                      <button
                        ...
                        if (subs.length === 0)
                          return (
                            <p className="text-sm text-gray-500 italic">
                              {t("no_workouts_on_day")}
                            </p>
                          );
                        return renderSubmissionsList(subs);
                      })()}
                    </div>
                  </div>
                )}
        ...
        {view === "PLANNING" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              <button
                onClick={() => setView("TRAINEE_DETAILS")}
                className="text-gray-500 hover:text-gray-800 flex items-center gap-1 text-sm font-medium"
              >
                <ChevronPrev size={16} /> {t("back_profile")}
              </button>
              <div className="flex items-center gap-2">
                <span className="text-sm text-gray-500">
                  {t("editing_program")}
                </span>
                <span className="font-bold text-lg bg-white px-3 py-1 rounded shadow-sm">
                  {users.find((u) => u.id === selectedTraineeId)?.name}
                </span>
              </div>
            </div>
                    ...
                    <div className="mb-3 p-3 bg-indigo-50 border border-indigo-100 rounded-xl space-y-2">
                      ...
                      <div className="flex gap-2">
                        <Button
                          ...
                          disabled={
                            isSavingExercise || !newExercise.name.trim()
                          }
                        >
                          {isSavingExercise ? (
                  ...
                  {exercises
                    .filter((ex) => {
                      ...
                      return (
                        ...
                        t(ex.name).toLowerCase().includes(q) ||
                    ...
                    .map((ex) => (
                      <div
                        key={ex.id}
                        className="group flex justify-between items-center p-3 hover:bg-gray-50 rounded-lg border border-transparent hover:border-gray-100 transition"
                      >
                        <div className="flex items-center gap-3">
                          <div className="text-indigo-400 bg-indigo-50 p-2 rounded-lg">
                            {getCategoryIcon(ex.category)}
                          </div>
                          <div>
                        ...
                        <button
                          onClick={() =>
                            !currentPlanExercises.includes(ex.id) &&
                            setCurrentPlanExercises([
                              ...currentPlanExercises,
                              ex.id,
                            ])
                          }
                          disabled={currentPlanExercises.includes(ex.id)}
                          className="text-indigo-600 hover:bg-indigo-50 p-1.5 rounded-md disabled:text-gray-300"
                        >
                          <PlusCircle size={20} />
                        </button>
                      </div>
              ...
              <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex flex-col overflow-hidden">
                <div className="p-4 border-b border-gray-100 bg-indigo-50/50 flex justify-between items-center">
                  ...
                  <span className="text-xs font-medium text-indigo-600">
                    {currentPlanExercises.length} {t("exercises_count")}
                  </span>
                </div>
                  ...
                  {currentPlanExercises.length === 0 && (
                    <div className="h-full flex flex-col items-center justify-center text-gray-400">
                      <div className="w-12 h-12 rounded-full bg-gray-100 flex items-center justify-center mb-2">
                        <Dumbbell className="opacity-50" />
                      </div>
                      <p className="text-sm">{t("no_exercises_assigned")}</p>
                    </div>
                  )}
                  {currentPlanExercises.map((id, index) => {
                    const ex = exercises.find((e) => e.id === id);
                    if (!ex) return null;
                    return (
                      <div
                        key={id}
                        className="flex justify-between items-center p-3 bg-white shadow-sm rounded-lg border border-gray-100"
                      >
                        <div className="flex items-center gap-3">
                        ...
                        <button
                          onClick={() =>
                            setCurrentPlanExercises(
                              currentPlanExercises.filter((x) => x !== id),
                            )
                          }
                ...
                <div className="p-4 border-t border-gray-100 bg-white">
                  <Button className="w-full" onClick={saveTraineePlan}>
                    {t("save_program")}
                  </Button>
                </div>
              </div>
        ...
        {view === "DASHBOARD" && user.role === UserRole.TRAINEE && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-indigo-600 to-blue-700 text-white rounded-2xl p-6 shadow-xl shadow-indigo-200">
          ...
          <div className="max-w-2xl mx-auto space-y-6">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-lg font-bold">
                {editingSubmissionId
                  ? t("edit_workout_session")
                  : t("new_workout_session")}
              </h2>
            </div>
            <div className="bg-white p-4 rounded-xl border border-gray-200">
              <h3 className="text-xs font-bold text-gray-500 uppercase tracking-wide mb-3">
                {t("selected_for_session")}
              </h3>
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200 space-y-4">
              <div className="flex justify-between items-center">
                ...
                <Badge color="bg-indigo-100 text-indigo-700">
                  {sessionMedia.length} {t("videos_count")}
                </Badge>
              </div>
              {sessionMedia.length === 0 ? (
                <div className="text-center py-8 border-2 border-dashed border-gray-100 rounded-xl bg-gray-50">
                  <Film className="mx-auto text-gray-300 mb-2" size={32} />
                  <p className="text-sm text-gray-400">{t("no_media_yet")}</p>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-3">
                  {sessionMedia.map((media) => (
                    <div
                      key={media.id}
                      className="rounded-lg overflow-hidden border border-gray-200 bg-gray-50 flex flex-col"
                    >
                      <video
                        src={media.preview}
                      ...
                      <div className="px-2 py-1.5 flex items-center justify-between bg-white border-t border-gray-100">
                        ...
                        <div className="flex items-center gap-1 shrink-0">
                          <button
                            onClick={() => setPlaybackUrl(media.preview)}
            ...
            <div className="bg-white p-4 rounded-xl border border-gray-200">
              <textarea
                className="w-full text-sm p-3 border rounded-lg focus:ring-2 focus:ring-indigo-100 outline-none resize-none h-24"
                placeholder={t("add_notes")}
                value={traineeNote}
                onChange={(e) => setTraineeNote(e.target.value)}
              />
            </div>
            <Button
              className="w-full h-14 text-lg shadow-xl shadow-indigo-200"
              onClick={handleFinishWorkout}
              disabled={sessionMedia.length === 0}
            >
              {editingSubmissionId ? t("update_workout") : t("finish_workout")}
            </Button>
          </div>
        )}
        ...
        {view === "RECORD" && (
          <div className="max-w-2xl mx-auto space-y-4">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-gray-300">|</span>
              <h2 className="text-lg font-bold">{t("session_recording")}</h2>
            </div>
            <VideoRecorder
              onSave={handleAddRecordedVideo}
              onCancel={() => setView("SESSION_HUB")}
              labels={{
                ready: t("ready"),
                rec: t("rec"),
                retake: t("retake"),
        ...
        {view === "HISTORY" && (
          <div className="space-y-6">
            <div className="flex items-center gap-4">
              ...
              >
                <ChevronPrev size={16} /> {t("back")}
              </Button>
              <h2 className="text-2xl font-bold">{t("workout_history")}</h2>
            </div>
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="overflow-x-auto">
                <table className="w-full text-sm text-left rtl:text-right">
                  <thead className="bg-gray-50 text-gray-500 font-medium border-b border-gray-200">
                    <tr>
                      ...
                      {user.role === UserRole.COACH && (
                        <th className="px-6 py-4">{t("col_trainee")}</th>
                      )}
                      <th className="px-6 py-4">{t("col_exercises")}</th>
                      <th className="px-6 py-4">{t("col_status")}</th>
                      <th className="px-6 py-4">{t("col_feedback")}</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-gray-100">
                    {submissions
                        ...
                        >
                          ...
                          {user.role === UserRole.COACH && (
                            <td className="px-6 py-4 font-medium text-gray-900">
                              {users.find((u) => u.id === sub.traineeId)?.name}
                            </td>
                          )}
