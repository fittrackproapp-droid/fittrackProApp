    ...
    const threads = partnerIds
      .map((pid) => {
        ...
        const threadMsgs = messages.filter(
          (m) => m.senderId === pid || m.receiverId === pid,
        );
      ...
      .filter(
        (t): t is { partner: User; lastMsg: Message; unreadCount: number } =>
          t !== null,
      )
        ...
        {threads.map((thread) => (
          ...
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h4
                  className={`font-bold ${thread.unreadCount > 0 ? "text-gray-900" : "text-gray-700"}`}
                >
                  {thread.partner.name}
                </h4>
                <p
                  className={`text-sm max-w-[200px] md:max-w-md truncate ${thread.unreadCount > 0 ? "text-gray-900 font-medium" : "text-gray-500"}`}
                >
                  {thread.lastMsg.senderId === currentUser.id ? "You: " : ""}
            ...
            >
              <div className="flex justify-between items-start mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <p
                      className={`text-sm ${isUnread ? "font-black text-gray-900" : "font-bold text-gray-700"}`}
                    >
                      {isMe ? "Me" : senderName}
                    </p>
                ...
                {isUnread && (
                  <span className="bg-indigo-600 text-white text-[9px] font-black px-2 py-0.5 rounded-full uppercase tracking-tighter shadow-sm animate-pulse">
                    New
                  </span>
                )}
              </div>
              <p
                className={`text-sm leading-relaxed whitespace-pre-wrap pl-11 ${isUnread ? "text-gray-900 font-semibold" : "text-gray-600"}`}
              >
                {msg.content}
              </p>
            </div>
